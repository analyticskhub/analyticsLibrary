...
var sVerNum = '161212'; // YYMMDD
/************************ ADDITIONAL FEATURES ***********************/
/*
var currentURL = window.location.href.toLowerCase();
if (currentURL.indexOf("stgeorge.com") != -1 || currentURL.indexOf("sgb.com.au") != -1) {
    var s_account = "stgeorge-prd"
} else {
    var s_account = "stgeorge-dev"
}
*/
var s_account = 'stgeorge-dev';
var isProdSite = (/^(?:www)\.stgeorge\.com\.au$/i).test(location.hostname);
if (isProdSite) {
    s_account = 'stgeorge-prd'; // moved to each call f
...
s.linkInternalFilters = "javascript:,stgeorge.com.au,stgeorge.com, sgb.com.au,stgeorgebank.com.au "
...
function s_doPlugins(s) {
    ...
    if (s.pageName == s.siteID) s.pageName += ':home';

    /*** App pageNaming ***/
    if (s.pageName.indexOf("apply-now:ola.asp") != -1 && s.getQueryParam("product")) {
        s.pageName = s.siteID + ":apply-now:ola:" + s.getQueryParam("product")
    }

    if (s.pageName.indexOf("apply-now:home") != -1 && s.getQueryParam("apptype")) {
        if (s.getQueryParam("DefaultCard")) {
            s.pageName = s.siteID + ":apply-now:ola:cca - " + s.getQueryParam("DefaultCard")
        } else {
            s.pageName = s.siteID + ":apply-now:ola:" + s.getQueryParam("apptype")
        }
    }
    ...
    if (!s.hier1) s.hier1 = s.pageName.replace(new RegExp(/:/g), "|");

    /*** error pages ***/
    if (typeof (s.linkType) == 'undefined') s.prevPg = s.getPreviousValue(s.pageName, 'gpv_pN');
    if (s.pageName == '404 error page') {
        s.prop32 = window.location.href;
        s.prop33 = s.prevPg;
    }
    ...
    /* Channel Manager */
    s.channelManager('cid', '', 'cm_g', '', 's_dl');
    //Only set the eVars if the channel manager plugin has returned a channel
    if (s._channel) {
        s.mailRef = s._referringDomain.indexOf('.mail.')
        if (s.mailRef > -1) s._referringDomain = s._referringDomain.substring(s.mailRef + 1);
        //set eVar33 and eVar37 equal to the channel.
        s.eVar33 = s.eVar37 = s._channel;
        //set eVar34 and eVar38 equal to the search engine used
        s.eVar34 = s.eVar38 = s._partner;
        //If the channel is paid search but no search engine is in the referrer, set eVar34/eVar38 equal to "Unknown Search Engine"
        if (s.eVar33 == 'Paid Search' && s.eVar34 == 'n/a') s.eVar34 = s.eVar38 = 'Paid Search - Unknown Search Engine';
        //Remove plus signs from keywords where plus signs weren't originally used
        s._keywords = s.repl(s._keywords, '+', ' ');
        s._keywords = s.repl(s._keywords, '  ', ' +');
        //Set eVar35 and eVar39 equal to the keywords used in the search engine
        s.eVar35 = s.eVar39 = s._keywords;
        //If the channel is paid search but no search keyword is in the referrer, set eVar35/eVar39 equal to "Unknown Keyword"
        if (s.eVar33 == 'Paid Search' && s.eVar35 == 'n/a') s.eVar35 = s.eVar39 = 'Paid Search - Unknown Keyword';
        //Designate the channel in the search engine/keywords reports if search wasn't used
        if (s.eVar33 && s.eVar34 == 'n/a') {
            s.eVar34 = s.eVar38 = 'n/a: ' + s.eVar33;
            s.eVar35 = s.eVar39 = 'n/a: ' + s.eVar33;
        }
        //Set eVar36 and eVar40 equal to the referring domain
        s.eVar36 = s.eVar40 = s._referringDomain;
        //Set eVar29 equal to the Unified Sources campaign value
        s.eVar29 = s._campaign;
    }
    /* Internal Search
    ...
    s.clickPast(s.SEMvar, 'event45', 'event46'); //can use any events available
