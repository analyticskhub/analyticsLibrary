...
(function (win, doc) {
	...
	helpers = {
		...
		pageSetup : function () {
			...
			omnitureScriptSource = testTracking.location ? 's_code_dev.js' : '//www.westpac.com.au/system/config/analytics/s_code_www.js',
			...
			// standard subdomains of westpac.com.au that should be prod
			// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
			// to switch on live host only
			if (/^(?:help|ww2)\.westpac\.com\.au$/i.test(location.hostname)) { // help and ww2 due to incorrect links in COL pages (should link to www)
		...
		send : {
			...
			page : function (details) {
				...
				if (!pageDetails.siteVersion) {
					pageDetails.siteVersion = 'analytics_www.js:20150730'; // TODO: ------ Code release date to be updated with changes
				}
