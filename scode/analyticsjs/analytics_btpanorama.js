...
(function (win, doc) {
	...
	helpers = {
		...
		pageSetup : function () {
			...
			omnitureScriptSource = testTracking.location ? 's_code_btpanorama.js' : '/vendors/adobe/omniture/s_code_btpanorama.js',
			...
			// BT domain details
			// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
			//btfgRegex = /^(www\.|btsfl\.|managedaccounts\.)?(bt|panorama|panoramainvestor|panoramaadviser|btadvisercentre|gsso\.intranet\.westpac)\.com\.au$/i;
			btfgRegex = /^(www\.|btsfl\.|managedaccounts\.)?(bt|panorama|panoramainvestor|panoramaadviser|btadvisercentre)\.com\.au$/i; // intranet not prod?
			btFundsRegex = /^online\.btfunds\.com\.au$/i;
			// to switch on live host only
			if (btfgRegex.test(location.hostname) || btFundsRegex.test(location.hostname)) {
				pageConfig.prod = true;
			}
			//if (window.org.bt.environment == 'LOCALHOST') {
			//	pageDetails.s_abort = true; //zzz: remove before deploy
			//}
			
			if (btfgRegex.test(helpers.location.hostname)) {
				pageConfig.brand = 'bt';
			...
			if (btFundsRegex.test(helpers.location.hostname)) {
				pageConfig.brand = 'btfunds';
				pageConfig.site = 'online';
			}
		...
		send : {
			...
			page : function (details) {
				...
				if (!pageDetails.siteVersion) {
					pageDetails.siteVersion = 'analytics_btpanorama.js:20150317'; // TODO: ------ Code release date to be updated with changes
				}
