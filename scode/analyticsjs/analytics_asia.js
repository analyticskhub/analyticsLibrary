...
(function (win, doc) {
	...
	helpers = {
		...
		pageSetup : function () {
			...
			omnitureScriptSource = testTracking.location ? 's_code_asia.js' : '//info.westpac.com.au/furniture/scripts/s_code_asia.js',
			...
			// Westpac Asia sites
			// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
			wbcAsiaRegexProd = /^(www\.)?westpac(\.asia|\.hk|\.in|bank\.com\.cn|\.com\.sg)$/i;
			wbcAsiaRegexAny = /\.?westpac(\.asia|\.hk|\.in|bank\.com\.cn|\.com\.sg)$/i;
			wbcAsiaSplit = helpers.location.hostname.split('.');
			// to switch on live host only
			if (wbcAsiaRegexProd.test(location.hostname)) {
				pageConfig.prod = true;
			}
			// for live host and test page
			if (wbcAsiaRegexAny.test(helpers.location.hostname)) {
				pageConfig.brand = 'wbc-' + wbcAsiaSplit[wbcAsiaSplit.length - 1];
				if (/^westpac(bank)?$/i.test(wbcAsiaSplit[0])) {
					pageConfig.site = 'www';
				} else {
					pageConfig.site = wbcAsiaSplit[0];
				}
			}
			// specify first-party cookie domain periods if not 3
			if (/\.?westpac(\.asia|\.hk|\.in)$/i.test(helpers.location.hostname)) {
		...
		send : {
			...
			page : function (details) {
				...
				if (!pageDetails.siteVersion) {
					pageDetails.siteVersion = 'analytics_asia.js:20141112'; // TODO: ------ Code release date to be updated with changes
				}
