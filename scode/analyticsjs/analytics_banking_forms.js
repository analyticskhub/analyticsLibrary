...
(function (win, doc) {
	...
	helpers = {
		...
		pageSetup : function () {
			...
			omnitureScriptSource = testTracking.location ? 's_code_dev.js' : '//info.westpac.com.au/furniture/scripts/s_code_banking_forms.js',
			omnitureScriptReady = function () {
				return win.s && win.s.w_trackPage;
			},
			// customisations for sites using this file
			wbcBankingFormsDomain,
			wbcBankingFormsPath;
			
			// In-session URLs for banking forms platform
			// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
			wbcBankingFormsDomain = /(^|.+\.)banking\.westpac\.com\.au$/i;
			wbcBankingFormsPath = /^(\/olfma\/|\/gcc\/currencycard\/wol\/|\/mpdm\/secure\/)/i;
			// for banking forms auth URLs
			if (wbcBankingFormsDomain.test(helpers.location.hostname) && wbcBankingFormsPath.test(helpers.location.pathname)) {
				pageConfig.authUrl = true;
			}
		...
		send : {
			...
			page : function (details) {
				...
				if (!pageDetails.siteVersion) {
					pageDetails.siteVersion = 'analytics_banking_forms.js:20150311'; // TODO: ------ Code release date to be updated with changes
				}
